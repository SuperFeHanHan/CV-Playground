...
def classify(dataset, idx, model, processor):
    ...
    with torch.no_grad():
        inputs = processor(images=img2, return_tensors="pt").to(model.device)
    ...
    for score, label_idx, box in zip(results["scores"], results["labels"], results["boxes"]):
        ...
        # 展示label和对应的分数
        draw2.text((bbox[0], bbox[1] - 10), text=f"{label}-conf={score:.2f}", fill=color)
...
if __name__=="__main__":
    ...
    MODEL_PATH = os.path.join(f"{ROOT_DIR}", "models", "detr", "e=40-s=34560-epoch_loss=1.41.pth")
    # dataset = DocLayNetDataset(os.path.join(ROOT_DIR, "data", "DocLayNet", "base_dataset", "train"))
    dataset = DocLayNetDataset(os.path.join(ROOT_DIR, "data", "DocLayNet", "base_dataset", "val"))
    ...
    pretrained_config.label2id = dataset.label2id  # 新的id -> label
    pretrained_config.id2label = dataset.id2label
    ...
    while True:
        ...
        classify(dataset, idx, model, processor)
